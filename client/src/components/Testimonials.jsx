            ...
            {testimonialsData.map((testi,index) => (
                <div key={index} className='bg-white/20 p-12 rounded-lg shadow-md border w-80 m-auto cursor-pointer hover:scale-[1.02] transition-all'>
                    <div className='flex flex-col items-center'>
                        <img src={testi.image} alt='' className='rounded-full w-14'></img>
                        <h2 className='text-xl font-semibold mt-3'>{testi.name}</h2>
                        <p className='text-gray-600 mb-4'>{testi.role}</p>
                        <div className='flex mb-4'>
                            {Array(testi.stars).fill().map((item,index)=>(
                                <img key={index} src={assets.rating_star} alt=''></img>
                            ))}
                        </div>
                        <p className='text-center text-sm text-gray-600'>{testi.text}</p>
                    </div>
                </div>
            ))}
        </div>
